...
ROS_SOURCES_PARAM_NAME = "rosSources"
...
def generate_overlay(expressions: dict[str, str], args, our_cmd_line: str):
    with file_writer(f'{args.output_dir or "."}/overlay.nix', args.compare) as f:
        print(f"# Automatically generated by: {our_cmd_line}\n", file=f)
        if args.fetch_in_flake_inputs:
            print(ROS_SOURCES_PARAM_NAME + ": final: prev:\n{", file=f)
        else:
            print("final: prev:\n{", file=f)
        for pkg in sorted(expressions):
            ...
            if args.fetch_in_flake_inputs:
                print(
                    f"  {pkg} = final.callPackage ./{expr} {{ inherit {ROS_SOURCES_PARAM_NAME}; }};",
                    file=f,
                )
            else:
...
def ros_distro_overlays_def(args):
    flake_input_sources = f" {ROS_SOURCES_PARAM_NAME} " if args.fetch_in_flake_inputs else ""
    return dedent(
        f"""
    applyDistroOverlay =
    ...
    rosDistroOverlays = final: prev: {{
      ...
      rosPackages = applyDistroOverlay (import ./overlay.nix {flake_input_sources}) prev.rosPackages;
    }};
"""
    ).strip()
...
def generate_flake(args, repos_of_packages: dict[str, dict[str, str]]):
    ...
    ros_inputs = '\n'.join(
        [
            generate_ros_input(
                repo, info["owner"], info["rev"] if args.use_flake_input_rev else None
            )
            for repo, info in repos_of_packages.items()
        ]
    )
    repo_list = list(repos_of_packages.keys())
    ros_sources = (
        f'''{ROS_SOURCES_PARAM_NAME} = {{ '''
        + ''.join([f'''{repo} = {repo};''' for repo in repos_of_packages.keys()])
        + ' };'
    )
...
'''
      ...
      in {{
        ...
        packages = builtins.intersectAttrs (import ./overlay.nix{flake_input_sources}null null) pkgs.rosPackages.${{rosDistro}};
        checks = builtins.intersectAttrs (import ./overlay.nix{flake_input_sources}null null) pkgs.rosPackages.${{rosDistro}};
...
def ros2nix(args):
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Determines the package structure from git, but the code is passed as a set of flake inputs. "
        "Requires --flake or --no-default.",
    )
    ...
    parser.add_argument(
        ...
        "Use with --fetch or --fetch-in-flake-inputs if some package.xml files are outside of the flake repo",
    )
    parser.add_argument(
        "--default",
        action=argparse.BooleanOptionalAction,
        help="Enforce/suppress generation of default.nix. The default.nix will not be created when --fetch-in-flake-inputs is set.",
    )
    parser.add_argument(
        "--overlay",
        action=argparse.BooleanOptionalAction,
    ...
    fetch_git = args.fetch or args.fetch_in_flake_inputs
    ...
    for source in args.source:
        try:
            ...
            if args.src_param:
                ...
                match = re.match(
                    r"https://(?:x-access-token:[^@]*@)?github\.com/(?P<owner>[^/]*)/(?P<repo>.*?)(?:\.git|/.*)?$",
                    url,
                )
                sparse_checkout = (
                ...
                elif args.fetch_in_flake_inputs:
                    kwargs["src_param"] = ROS_SOURCES_PARAM_NAME
                    kwargs["src_expr"] = f"{ROS_SOURCES_PARAM_NAME}.{match['repo']}"
                    repos.update(
                        {match["repo"]: {"owner": match["owner"], "rev": info["rev"]}}
                    )  # makes sure that we dont have the same repo multiple times
                else:
        ...
        try:
            ...
            for patch in patches:
                ...
                if not patch_filename in patch_filenames:
                    ...
                    raise Exception(msg)
                with (
                    file_writer(patch_filename, args.compare) as patch_dest,
    ...
    if args.overlay:
        generate_overlay(expressions, args, our_cmd_line)
    ...
    if not args.fetch_in_flake_inputs and (
        args.default or (args.default is None and not args.flake)
    ):
